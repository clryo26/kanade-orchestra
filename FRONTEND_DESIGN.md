...
- 管理側と団員側の演奏会情報では、曲目一覧を略称ではなく正式名称（作曲者: 曲名）で表示し、曲情報の「部」は曲名表示に付けない
- 管理側の曲目一覧では、曲名下に略称・部・演奏時間を1行で補助表示する
...
- 団員向け「練習予定」の各カードは日付の下に「練習場所: 会場名」を表示する。保存済みの `venue` をHTMLエスケープして表示し、未登録の場合は「未定」とする。
...
- 団員画面の曲一覧では説明見出しを出さず、曲名は略称ではなく正式名称（作曲者: 曲名）で表示し、曲情報の「部」は曲名表示に付けない
...
- 団員画面の曲一覧では曲名を略称ではなく正式名称（作曲者: 曲名）で表示し、曲情報の「部」は曲名表示に付けない
