...
現在のWebポータル実装の詳細仕様は [DESIGN_WEB.md](DESIGN_WEB.md) の先頭にある「現行実装仕様メモ（2026-06-13反映）」を正とする。

現行実装では、当初の録音共有ツールから団員ポータルへ機能拡張している。
...
- アルバム（写真共有）
- 団員紹介ページへの導線
- 録音一覧の遅延読み込みとaudio要素の遅延生成
- Google Cloud Storage連携
- 汎用ポータルデータAPI `/api/portal/{collection}`
- ポータルファイルAPI `/api/portal-files/{kind}`

注意:

- 管理者パスワードロックはフロントエンド側の簡易ロックであり、本番運用ではAPI側の認証が必要。
- 支払状況などの個人情報を含む機能は、公開前に閲覧権限、認証、監査ログを追加すること。
