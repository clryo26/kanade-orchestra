...
- 団員名 + パート + パスワードでログイン
- ログイン時の名前/パート照合は、全角半角揺れ（NFKC）と不可視文字（ゼロ幅スペース等）を正規化して照合する
- 団員パスワードはハッシュ保存し、画面/APIでは復元表示せず設定済み/未設定のみ表示する
...
- 練習指示管理
- 団員登録
- 団員登録のパスワード欄は再設定用とし、未入力保存では既存パスワードを保持する
- 支払状況登録
- 会場管理
- 乗り番管理
- 楽譜管理
...
- 接続先情報
- パート管理
- アクセスログ: ログイン済み団員がいつ何のメニューへアクセスしたかをシステム管理者が確認できる
...
- 曲目管理: 曲名、略称、作曲者、部、演奏時間、アンコール属性、曲順
- 演奏会情報管理と団員向け演奏会情報の曲目一覧は略称ではなく正式名称（作曲者: 曲名）を表示し、曲情報の「部」は曲名表示に付けない。管理画面では上へ/下へ操作で表示順を編集でき、曲名下に略称・部・演奏時間を1行で補助表示する
- チラシ画像登録は曲目一覧の下に配置し、曲目入力の流れを妨げない
...
- 時刻表示は秒を出さず `HH:MM` 形式に統一する
- 指揮トレ、本番、備考
- Google カレンダー連携、ICS 出力

### 6.2 本番情報

- 管理者メニューの「本番情報管理」で演奏会ごとに次の3項目を登録する
  - 当日のタイムテーブル
  - 本番衣装（男女別: 男性/女性それぞれ 上・下・その他）
  - 係り割
- 「部ごとのリハ時間」は時刻ではなく分数として登録し、000〜999 の3桁数字で表示する。1桁または2桁入力は保存時にゼロ埋めする（例: 12 → 012）
- 団員メニューの「本番情報」で、演奏会ごとに登録済み内容を参照する
- 団員メニューの本番情報は、演奏会日付の新しい順で表示する
- 係り割は管理画面で「担当 / 氏名」の表形式入力（行追加）で登録する

### 6.3 録音管理

- 録音アップロード時の曲名は、選択した演奏会に登録されている曲目からコンボボックスで選択する
- 録音アップロード時の曲名コンボボックスは正式名称（作曲者: 曲名）を表示し、保存フォルダ名と録音一覧表示には略称を使用する
- 録音アップロード時の曲名コンボボックスには、曲単位に分類しない録音用として「練習全体の通し」を追加表示する
- 録音の保存フォルダ名と画面表示の曲名は、曲目に略称が登録されている場合は略称を優先する

- 音声アップロード（ローカル + Cloud）
- 一覧表示、再生、ダウンロード、ZIP 一括 DL
- Cloud とローカルに同一録音が存在する場合、録音一覧は Cloud 側を優先して 1 ファイル 1 件で表示する
- GCS 録音メタデータは `object_name` を外部識別子とし、PostgreSQL の `drive_files.id` は内部採番IDとして扱う
- GCS 録音メタデータ保存時は `drive_files.created_at` / `updated_at` を必ず補完する
- Cloud 配信時の Range 対応
- 再生時間メタデータ保持
- 団員向け録音部屋の連続再生は、単一の audio 要素をプレイリストとして使い回し、スマートフォンのバックグラウンド再生中も曲終了時に次の録音へ自動遷移する
- スマートフォン表示では、連続再生のチェック項目を Bootstrap の既定余白と競合しない boxed レイアウトにし、左枠線とチェックボックスが重ならないようにする

### 6.4 楽譜管理

- 演奏会/曲/PDF 登録
- 楽譜一覧の絞り込み表示
- 楽譜ライブラリの演奏会名・曲名見出しは左寄せ表示
- 単体・曲単位・演奏会単位の DL
- 楽譜パート設定（単票/一括）
- PDF ビューア（拡大縮小、幅合わせ）
...
- 団員本人の欠席/遅刻/早退登録
- 遅刻/早退の予定時刻を保存し、本人編集と一覧表示で参照する
- スマートフォン表示では、欠席連絡の入力欄と編集後の一覧行が viewport 幅を超えないようにし、iOS Safari の自動ズームを誘発しやすい小さな文字サイズを避ける
- 練習日単位の可視化
- 本人編集/削除

### 6.6 支払状況

- 支払設定: 月額団費（org_settings.membership_fee_amount）、演奏会費（performances.performance_fee_amount）
- 支払管理: 団費支払い月（paid_until_month）、最新支払日（latest_payment_date）、演奏会費支払い有無（performance_fees）
- 支払管理では団員費・演奏会費の金額登録は行わず、支払設定で登録した金額を表示する
- 団員向け支払状況では、団費はログイン中団員の支払済み月、演奏会費は演奏会ごとの支払済み/未払いのみを表示する
- 滞納警告表示

### 6.7 乗り番管理

- 演奏会単位の乗り番レコード
- 登録済み乗り番一覧の「編集」ボタンは対象レコードをフォームへ表示し、曲名、団員、エキストラを編集できる
- 団員リストとエキストラリストを別管理
- エキストラ情報: 名前、ふりがな、パート
- ポータル表示: 演奏会ごと・曲ごとにセクション分割し、出演者をパートごとグルーピングして表形式表示
  - 団員名は各パート内で縦並び表示
  - パート名と団員名は近接して表示し、団員名セルを左寄せにする
...
- 団員側の曲一覧では曲名を略称ではなく正式名称（作曲者: 曲名）で表示し、曲情報の「部」は曲名表示に付けない
- 曲詳細の初期表示は閲覧専用とし、「編集」ボタン押下で編集可能にする。保存後は再び閲覧専用へ戻す
...
- 団員側の曲一覧では説明見出しを出さず、曲名は略称ではなく正式名称（作曲者: 曲名）で表示し、曲情報の「部」は曲名表示に付けない
- 曲名を選択すると、選択した曲の練習時の指摘内容を詳細表示
- 曲詳細の初期表示は閲覧専用とし、「編集」ボタン押下で編集可能にする。保存後は再び閲覧専用へ戻す
- ポータルメニューから「練習指示」に入る場合は、直前の曲詳細状態を引き継がず毎回曲一覧を表示
...
- フロントは `updateCloudRunRevision()` でログイン画面とドロワーの Rev. 表示を動的更新し、`kanade-orchestra-00060-hsf` のような値は `00060-hsf` と表示する
- ログイン画面にも更新ボタンを配置し、未ログイン状態で最新リビジョンへ再読み込みできる
...
- ID/ログイン名インデックス
- bootstrap 系 ETag 応答
- bootstrap 系 ETag は performances 単体ではなく、レスポンスに含まれる全コレクションの合成値を使う
...
## PostgreSQL移行後のシステム管理メニュー

- JSON->PostgreSQL移行完了後の現行アプリでは、移行実行メニューとデータメンテナンスメニューを表示しない。
