...
- `npm run check:workflow:static` は対象WorkflowをPyYAMLでパースし、YAML構文エラーを検出する。
- `run:` ブロックへ `${{ secrets.* }}` を直接記載する場合は、出力コマンド種別にかかわらず失敗させる。
- `env:` でSecretを渡す既存の方式は許可し、実値をログ出力しない。
