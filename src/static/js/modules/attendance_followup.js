...
function attendanceOverviewHtml(schedule, members) {
    ...
    return `<article class="info-block attendance-overview" data-attendance-overview>
        <strong>${escapeHtml(formatScheduleDate(schedule.date))} ${escapeHtml(scheduleTimeLabel(schedule) || '')}</strong>
        <div class="small text-muted mb-2">${escapeHtml(schedule.venue || '')}</div>
