...
@app.put("/api/system/role-permissions")
async def upsert_role_permission(
    ...
    permission_key = body.permission_key.strip()
    ...
    with psycopg.connect(db_connection_string(), autocommit=True) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (role_name, permission_key, body.enabled),
            )
    return {"status": "ok", "role_name": role_name, "permission_key": permission_key, "enabled": body.enabled}
