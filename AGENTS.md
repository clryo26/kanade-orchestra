...
このリポジトリでAIエージェントが作業する場合の作業契約を定義する。

## 役割

AIエージェントは、実装支援・調査支援・レビュー支援・テスト観点作成を行う。  
AIエージェントは、最終品質保証・完了判定・本番リリース判断を行わない。

## 作業順序

AIエージェントは、コードを書く前に必ず以下の順序を守る。
...
- 実行していない確認を実施済みと書くこと
- 未確認事項を隠すこと
- 完了、確認済み、問題なし、と断定すること

## 修正差分の原則

修正は最小差分とする。  
大規模な書き換え、構造変更、命名変更、共通化は、依頼に明記されていない限り行わない。
...
判定だけを書いてはいけない。必ず根拠を書く。

## 未確認事項

実際に実行していない内容は、必ず未確認と書く。
