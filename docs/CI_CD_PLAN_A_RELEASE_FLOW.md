...
## 9. pushからテスト環境反映までの標準自動化

コード変更、必要なローカルテスト、コミットまでが完了した後は、push以降の標準手順として次のPowerShellスクリプトを使用する。

```powershell
.\scripts\deploy_to_test.ps1
```

ユーザーが「テスト系に反映して」「テスト環境へ反映して」「プッシュ以降を進めて」など同じ趣旨を指示した場合、チャットまたはVS Codeエージェントはこのスクリプトを優先して実行する。

担当範囲:
1. 作業ブランチ・作業ツリー・必要コマンド・認証状態の確認
2. 現在ブランチが最新のorigin/mainを含むことの確認
3. 現在ブランチのpush
4. PRの再利用または新規作成
5. PR CI成功確認
6. PRのmainへのmerge
7. main CI成功確認
8. Deploy Test成功確認
9. TEST Cloud RunのReady Revision、GIT_SHA、image digest、100% traffic確認

失敗時はその時点で停止し、後続工程を実行しない。
...
「テスト系に反映して」という指示はTEST環境への反映のみを許可するものであり、本番反映、DB同期、GCS同期の許可を兼ねない。
