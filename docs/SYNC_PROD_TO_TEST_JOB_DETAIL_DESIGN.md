...
## 17. GCS同期詳細
- `scripts/sync_prod_to_test_gcs.py` が固定対象 `sheets/`・`albums/` と、バケット直下の有効な日付形式録音フォルダーを同期する。
- 既定動作は dry-run とし、コピー・削除予定を出力する。`--execute` 指定時だけテストバケットを更新する。
- コピー元とコピー先のサイズ・CRC32C（取得できない場合はMD5）を比較し、内容が同じオブジェクトは更新しない。
- 新規作成・上書き・削除はGCSの世代条件を指定し、列挙後に変更されたオブジェクトを誤更新しない。
- 本番に存在しないテスト側の同期対象は、全コピーの検証完了後に削除する。
- 実行後は同期対象のオブジェクト名と内容を再列挙して本番側との一致を検証する。
- `app-data/`、設計済み除外プレフィックス、`recordings/`、`promotion/`、日付形式に一致しないバケット直下フォルダーは読み書きしない。
- 現段階では独立実装のみとし、GitHub Actionsワークフローへの接続は行わない。
